...
\begin{abstract} % put abstract here.
  ...
  In practice, the joystick approach is \(42 \%\) faster on average on a task that involves moving the end effector to a random position in the workspace.
  Pose estimation is faster than joystick control by \(60.3 \%\) in a scenario involving grabbing an object in one location and moving it to a target destination.
  In a similar scenario involving an obstacle field, the pose estimation solution is \(57.2 \%\) faster and has \(3\) fewer collisions on average.
...
\label{sec:conc}
  ...
  The results of the experiments shows that in the arbitrary movement task, joystick control results in a \(42 \%\) faster average speed to reach a random position.
  ...
  However, the use of pose estimation had increased performance in the two pick and place scenarios, having an average time of \(60.3 \%\) and \(57.2 \%\) respectively.
